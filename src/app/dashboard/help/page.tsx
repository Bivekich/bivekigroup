...
import { Home, User, Bell, Globe, HelpCircle, Cloud } from 'lucide-react';
...
const categories: Category[] = [
  ...
  },
  {
    ...
    questions: [
      {
        q: 'Как работает оплата облачных услуг?',
        a: 'Оплата облачных услуг производится ежедневно в 23:59 по московскому времени. Средства списываются с вашего баланса автоматически. Минимальная сумма пополнения баланса - 100 ₽. Вы можете пополнить баланс с помощью банковской карты или выставить счет на оплату.',
      },
      {
        q: 'Какие тарифы доступны?',
        a: 'Мы предлагаем различные тарифы для облачных серверов и баз данных. Для просмотра доступных тарифов нажмите на кнопку "Тарифы" рядом с балансом. В тарифах указана стоимость за месяц использования, характеристики (CPU, RAM, диск) и дополнительные опции.',
      },
      {
        q: 'Как подключить облачные услуги?',
        a: 'Для подключения облачных услуг необходимо обратиться к менеджеру. Он поможет выбрать оптимальный тариф под ваши задачи и проконсультирует по всем вопросам.',
      },
    ],
  },
];
